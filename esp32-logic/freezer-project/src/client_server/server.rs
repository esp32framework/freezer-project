use esp32framework::{ble::{utils::{ble_standard_uuids::{StandardCharacteristicId, StandardServiceId}, Characteristic, Service}, BleId, BleServer}, wifi::{http::{Http, HttpClient, HttpsClient}, WifiDriver}, Microcontroller};
    ...
    fn from(client_id: u16, characteristics_data: Vec<(BleId, Vec<u8>)>)-> Option<Self>{
        ...
        for (c_id, data) in characteristics_data {
            match c_id {
                ...
                CHAR_TEMPERATURE_ID => temperature = Some(f32::from_be_bytes(data.try_into().ok()?)),
                _ => {}
            }
        }

        Some(ClientData{id: client_id, temperature: temperature?,pressure: pressure?,humidity: humidity?})
    }

    fn print(&self){
        println!("[ESP_{}] Humidity: {}  |  Pressure: {}  |  Temperature: {} ", self.id, self.humidity, self.pressure, self.temperature);
    }

    fn send(&self, https_client: &mut HttpsClient){
        let send_data = format!("{}?id={}&hum={}&press={}&temp={}", URI, self.id, self.humidity, self.pressure, self.temperature);
        self.print();
...
fn initialize_ble_server<'a>(micro: &mut Microcontroller<'a>) -> BleServer<'a> {
    ...
    micro.ble_server(ADVERTISING_NAME.to_string(), &services).unwrap()
}

fn initialize_wifi_connection<'a>(micro: &mut Microcontroller<'a>) -> (WifiDriver<'a>, HttpsClient) {
